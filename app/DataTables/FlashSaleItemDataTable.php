    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function ($query) {
                $deleteBtn = "<a href='" . route('admin.flash-sale.destroy', $query->id) . "' class='btn btn-danger ml-2 delete-item'><i class='fa-regular fa-trash-can'></i></a>";
            ...
            ->addColumn('show_at_home', function ($query) {
                if ($query->show_at_home == 1) {
                    $button = '<label class="custom-switch mt-2">
                            <input type="checkbox" checked name="custom-switch-checbox" class="custom-switch-input change-at-home-status" data-id="' . $query->id . '">
                            <span class="custom-switch-indicator"></span>
                        </label>';
                } else {
                    $button = '<label class="custom-switch mt-2">
                            <input type="checkbox" name="custom-switch-checbox" class="custom-switch-input change-at-home-status" data-id="' . $query->id . '">
                            <span class="custom-switch-indicator"></span>
                        </label>';
                }
